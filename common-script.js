...
function postIngredient() {
  ...
  // Validate inputs
  if (!shopName || !ingredient || isNaN(quantity) || isNaN(price)) {
    alert("Please fill in all fields correctly.");
    return;
  }

  // Create an ingredient data object
  ...
  // Retrieve existing stored ingredients or initialize an empty array
  const storedIngredients = JSON.parse(localStorage.getItem("storedIngredients")) || [];
  ...
  alert(`Ingredient "${ingredient}" from "${shopName}" has been added successfully.`);

  // Reset the form
  document.getElementById("ingredient-form").reset();
}

// Function to load ingredients in stored_ingredients.html
function loadStoredIngredients() {
  const storedIngredients = JSON.parse(localStorage.getItem("storedIngredients")) || [];
  const ingredientListDiv = document.getElementById("stored-ingredient-list");

  if (storedIngredients.length === 0) {
    ingredientListDiv.innerHTML = `<p style="color:red">No new ingredients stored yet!!</p>`;
    return;
  }

  // Generate HTML for each ingredient
  ingredientListDiv.innerHTML = storedIngredients
    .map(
      (ingredient) => `
      <div class="ingredient-item">
        ...
        <p><strong>Available Quantity:</strong> ${ingredient.quantity} kg</p>
        <p><strong>Price:</strong> $${ingredient.price.toFixed(2)} per kg</p>
        <p><small><strong>Posted On:</strong> ${ingredient.postedOn}</small></p>
        <hr>
      </div>
    `
    )
...
// Automatically load stored ingredients when stored_ingredients.html is opened
document.addEventListener("DOMContentLoaded", () => {
  if (document.getElementById("stored-ingredient-list")) {
    loadStoredIngredients();
  }
});

function loadRestaurantIngredients() {
  const restaurantIngredients = JSON.parse(localStorage.getItem("restaurantIngredients")) || [];
  const ingredientListDiv = document.getElementById("ingredient-list");

  if (restaurantIngredients.length === 0) {
    ingredientListDiv.innerHTML = `<p>No ingredients are available yet.</p>`;
    return;
  }

  // Generate HTML to display ingredients
  ingredientListDiv.innerHTML = restaurantIngredients
    .map(
      (ingredient) => `
      <div class="ingredient-item">
        <p><strong>Shop Name:</strong> ${ingredient.shopName}</p>
        ...
        <p><small>Posted on: ${ingredient.postDate}</small></p>
        <hr>
      </div>
    `
    )
...
// Automatically load ingredients for the Restaurant Dashboard
document.addEventListener("DOMContentLoaded", () => {
  if (document.getElementById("ingredient-list")) {
    loadRestaurantIngredients();
  }
});
...
function placeOrder() {
  ...
  // Retrieve existing orders from localStorage for "My Orders"
  const orders = JSON.parse(localStorage.getItem("orders")) || [];
  orders.push(orderDetails);
  localStorage.setItem("orders", JSON.stringify(orders));

  // Save the order to "restaurantOrders" for restaurant orders display
  const restaurantOrders = JSON.parse(localStorage.getItem("restaurantOrders")) || [];
  restaurantOrders.push(orderDetails);
  localStorage.setItem("restaurantOrders", JSON.stringify(restaurantOrders));
